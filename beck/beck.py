...
def main(input_path: str, output_path: str):
    ...
    with mp_pose.Pose(static_image_mode=True, min_detection_confidence=0.5) as pose:
        ...
        draw.text((50, 40), f"Наклон плеч: {shoulder_angle:.1f}°", font=font_main,
                  fill=(255, 255, 0), stroke_fill=(0, 0, 0), stroke_width=1)
        draw.text((50, 80), f"Искривление позвоночника: {spine_angle_from_vertical:.1f}°",
                  font=font_main, fill=(0, 255, 255), stroke_fill=(0, 0, 0), stroke_width=1)
        draw.text((50, 130), f"Диагноз: {diagnosis}", font=font_diag,
                  fill=color, stroke_fill=(0, 0, 0), stroke_width=1)
        draw.text((30, h - 60),
                  "ВНИМАНИЕ: Результат не является медицинским диагнозом!",
...
if __name__ == "__main__":
    if len(sys.argv) != 3:
        print("Использование: python beck.py input.jpg output.jpg")
        sys.exit(1)
